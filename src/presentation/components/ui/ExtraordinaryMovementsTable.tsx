...
interface ExtraordinaryMovementsTableProps {
  movements: ExtraordinaryMovement[]
  accounts: Account[]
  onEdit: (m: ExtraordinaryMovement) => void
...
export default function ExtraordinaryMovementsTable({
  movements,
  accounts,
  onEdit,
  onDelete,
}: ExtraordinaryMovementsTableProps) {
  const accountMap = new Map(accounts.map((a) => [a.id, a]))

  if (movements.length === 0) {
    return (
      <div className="flex items-center justify-center h-48 text-text-secondary text-sm">
        No hay movimientos extraordinarios.
      </div>
    )
  }
